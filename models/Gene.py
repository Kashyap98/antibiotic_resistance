...
class Gene:
    ...
    def __init__(self, organism, gene_name, get_info=False):
        ...
        # if "-protein" not in self.fasta_file:
        #      self.fasta_file = self.fasta_file.replace(".fasta", "-protein.fasta")
        ...
        if get_info:
            if "-protein" in self.txt_file:
                self.txt_file = self.txt_file.replace("-protein", "")
